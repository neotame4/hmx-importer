...
def get_relative_pose(armature):
    data = {}
    for bone in armature.pose.bones:
        m = bone.matrix
        parent = bone.parent
        if parent:
            rel = parent.matrix.inverted() @ m
        else:
            rel = m.copy()
        data[bone.name] = {
            'pos': rel.to_translation(),
            'rot': rel.to_quaternion()
        }
    return data

def compute_delta(base, current):
    delta = {}
    for name in base.keys():
        if name not in current:
            continue
        base_pos = base[name]['pos']
        base_rot = base[name]['rot']
        cur_pos = current[name]['pos']
        cur_rot = current[name]['rot']
        delta_pos = cur_pos - base_pos
        delta_rot = base_rot.inverted() @ cur_rot
        delta[name] = {
            'pos': delta_pos,
            'rot': delta_rot
        }
...
#       hacky fix but it works
    ...
    if version >= 19:
        ...
        if version3 > 11:
            ...
            base_pose = get_relative_pose(armature)
            for i in range(num_samples):
                ...
                current_pose = get_relative_pose(armature)
                delta_pose = compute_delta(base_pose, current_pose)

                for name in char_bones:
                    stripped_name = name.replace(".pos",".mesh").replace(".quat",".mesh").replace(".rotz",".mesh")
#.replace(".mesh","")
                    bone = armature.pose.bones.get(stripped_name)
                    if not bone or stripped_name not in delta_pose:
                        continue
                    if ".pos" in name:
                        print("name", name)
                        name = name.replace(".pos", ".mesh")
                        bone = armature.pose.bones.get(name)
                        delta = delta_pose[name]
                        if compression < 2:
                    ...
                    if ".quat" in name:
                        ...
                        if bone:
                           ...
                           # bone.rotation_quaternion = (quat4, quat1, quat2, quat3)
                            bone.rotation_quaternion = delta['rot']
